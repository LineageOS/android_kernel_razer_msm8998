...
static void fwu_parse_image_header_05_06(void)
{
    ...
    pr_info("%s: fwu->img.contains_disp_config = %d\n",
            __func__, fwu->img.contains_disp_config);
...
static int fwu_parse_image_info(void)
{
    ...
    pr_info("%s: major header version (0x%02x)\n",
...
static enum flash_area fwu_go_nogo(void)
{
    ...
    if (fwu->bl_version == BL_V7 || fwu->bl_version == BL_V8)
        ...
        config_id_size = V5V6_CONFIG_ID_SIZE;
    image_config_id = le_to_uint(fwu->img.ui_config.data);
...
static int fwu_do_reflash(void)
{
    ...
    pr_info("%s: fwu_write_firmware\n", __func__);
    retval = fwu_write_firmware();
    if (retval < 0)
        return retval;
    pr_info("%s: Firmware programmed\n", __func__);

    fwu->config_area = UI_CONFIG_AREA;

    pr_info("%s: fwu->flash_properties.has_disp_config = %x, fwu->img.contains_disp_config = %d\n", __func__,
            fwu->flash_properties.has_disp_config, fwu->img.contains_disp_config);

    if (fwu->flash_properties.has_disp_config &&
            fwu->img.contains_disp_config)
    {
        pr_info("%s: fwu_write_dp_configuration\n", __func__);
    ...
    pr_info("%s: fwu_write_ui_configuration\n", __func__);
    retval = fwu_write_ui_configuration();
    if (retval < 0)
        return retval;
    pr_info("%s: Configuration programmed\n", __func__);
...
static int fwu_start_reflash(void)
{
    ...
    pr_info("%s: Start of reflash process\n", __func__);
...
exit:
    ...
    if (do_rebuild)
    {
        pr_info("%s: reset_device\n", __func__);
        rmi4_data->reset_device(rmi4_data, true);
    }

    pr_info("%s: End of reflash process\n", __func__);
...
static void fwu_show_config_id(void)
{
