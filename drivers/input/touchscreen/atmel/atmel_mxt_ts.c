...
//#include <../../drivers/fih/fih_touch.h> //SW-PROD-DH-Touch-00+
#ifdef CONFIG_FB
...
#include <linux/cyttsp3_ff_api.h> //SW-PROD-DH-ForceTouch-00+
#endif
...
#define DO_FIRMWARE_UPDATE  1
static char fih_touch[32] = {0}; //SW-PROD-DH-TOUCH-FTM-00+
...
#ifdef CONFIG_FB
static struct mxt_data *touch_data;
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
    ...
    {
        data->config_crc = crc;
        dev_err(dev, "T6 Config Checksum: 0x%06X\n", crc);
    }
    ...
    if (status != data->t6_status)
        dev_err(dev, "T6 Status 0x%02X%s%s%s%s%s%s%s\n",
                status,
                status == 0 ? " OK" : "",
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP3_DEVICETREE_SUPPORT
    int z; //SW-PROD-DH-ForceTouch-00+
#endif
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP3_DEVICETREE_SUPPORT
        z = _cyttsp3_force_function.force(dev, x, y, 0);//SW-PROD-DH-ForceTouch-00+
#endif
        input_report_abs(input_dev, ABS_MT_POSITION_X, x);
        input_report_abs(input_dev, ABS_MT_POSITION_Y, y);
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP3_DEVICETREE_SUPPORT
        input_report_abs(input_dev, ABS_MT_PRESSURE, z); //SW-PROD-DH-ForceTouch-00+
#else
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
    ...
    struct device *dev = &data->client->dev;//SW-PROD-DH-TOUCH-FTM-00+
    ...
    if (report_id == data->T6_reportid)
    {
        ...
        snprintf(fih_touch, PAGE_SIZE, "Atmel-V%06x", data->config_crc);
        dev_info(dev, "fih_touch = %s\n", fih_touch);
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg)
{
    ...
    if (info_crc == data->info_crc)
    {
        ...
        {
            dev_err(dev, "CRC zero, attempting to apply config\n");
        }
        else if (config_crc == data->config_crc)
        {
            dev_err(dev, "Config CRC 0x%06X: OK\n",
                    data->config_crc);
            return 0;
        }
        else
    ...
    if (data->T71_address)
        ...
        dev_err(dev, "Could not find CRC start\n");
    ...
    if (crc_start > cfg_start_ofs)
    {
        ...
        if (config_crc > 0 && config_crc != calculated_crc)
            dev_err(dev, "Config CRC in file inconsistent, calculated=%06X, file=%06X\n",
...
static int mxt_read_info_block(struct mxt_data *data)
{
    ...
    pr_err("### data->info_crc=%x ,calculated_crc=%x\n",data->info_crc,calculated_crc);
...
static int mxt_reset_setup(int gpio,const char *label)
{
    int retval = 0;
...
static int mxt_gpio_setup(int gpio,const char *label, bool configure, int dir, int state)
{
    int retval = 0;
    ...
    if (configure)
    {
        ...
        if (retval)
        {
            pr_err("%s: Failed to get gpio %d,name=%s (code: %d)",
                   __func__, gpio,label, retval);
            return retval;
        }
        ...
        if (retval)
        {
            pr_err("%s: Failed to set gpio %d,name=%s direction",
                   __func__, gpio,label);
            return retval;
        }
    }
...
static void mxt_regulator_enable(struct mxt_data *data)
{
    ...
    error = mxt_gpio_setup(data->pdata->gpio_reset,"atmel_rst",
                           true, 1, 0); //Low
    if(error < 0)
    {
        pr_err("[%s]:Failed to configure attention GPIO %d\n",
               __FUNCTION__,data->pdata->gpio_reset);
    }
    else
    {
        pr_err("[%s]: configure attention GPIO OK %d\n",
               __FUNCTION__,data->pdata->gpio_reset);
    ...
    gpio_free(data->pdata->ldo_gpio);
    error = mxt_gpio_setup(data->pdata->ldo_gpio,"atmel_ldo",
                           true, 1, 0);
//Enable regulator 1.8V
    gpio_free(data->pdata->ldo_gpio);
    error = mxt_gpio_setup(data->pdata->ldo_gpio,"atmel_ldo",
                           true, 1, 1);
    if(error < 0)
    {
        pr_err("[%s]:Failed to configure attention GPIO %d\n",
               __FUNCTION__,data->pdata->ldo_gpio);
        return ;
    }

//Enable regulator 3.3V
    error = regulator_enable(data->reg_vdd);
    if (error)
    {
        pr_err("reg_vdd enable fail \n");
        return;
    }
...
//RST control
    msleep(5);
    mxt_reset_setup(data->pdata->gpio_reset,"atmel_rst");
                                 ...
                                 const struct firmware *cfg)
{
    ...
    if (cfg)
    {
        ...
        if (error)
            dev_err(dev, "Error %d updating config\n", error);
    }
...
static const struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
    ...
    pdata->ldo_gpio = of_get_named_gpio(client->dev.of_node,
                                        "atmel,avdd-ldo-gpio", 0);
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP3_DEVICETREE_SUPPORT
    //SW-PROD-DH-ForceTouch-00+[
    cyttsp3_reset_baseline();
    dev_dbg(&client->dev, "Call cyttsp3_reset_baseline()\n");
    //SW-PROD-DH-ForceTouch-00+]
#endif
#ifdef CONFIG_FB
    touch_data = data;
    ...
    {
        pr_err("Register FB client failed!\n");
    }
#endif
    tp_probe_success = 1;
...
static int mxt_remove(struct i2c_client *client)
{
    ...
    regulator_put(data->reg_vdd);
#ifdef CONFIG_FB
    fb_unregister_client(&atmel_fb_notifier);
#endif
...
static int __maybe_unused mxt_resume(struct device *dev)
{
    ...
    //SW-PROD-DH-ForceTouch-00+[
    cyttsp3_reset_baseline();
    dev_dbg(dev, "Call cyttsp3_reset_baseline()\n");
    //SW-PROD-DH-ForceTouch-00+]
#endif
...
static int atmel_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && touch_data)
    {
        ...
        if (event == FB_EVENT_BLANK)
        {
            ...
            {
                pr_err("Screen On\n");
                mxt_resume(&touch_data->client->dev);
            }
            else if (*blank == FB_BLANK_POWERDOWN)
            {
                pr_err("Screen Off\n");
                mxt_suspend(&touch_data->client->dev);
            }
        }
    }
    return 0;
