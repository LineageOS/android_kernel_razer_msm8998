...
static int tusb544_parse_dt(struct device *dev, struct tusb544_platform_data *pdata)
{
	int r = 0;
			...
			const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		r = tusb544_parse_dt(&client->dev, platform_data);
		if (r)
			goto err_free_data;
	} else
