...
#ifdef CONFIG_MACH_RCL
	if (!ret)
		pr_info("%s: Request to switch to %d,",
...
static inline void dfps_update_fps(struct mdss_panel_info *pinfo,
				   u32 fps)
{
	if (pinfo->type == DTV_PANEL)
...
static int calc_extra_blanking(struct mdss_panel_data *pdata,
			       u32 new_fps)
{
