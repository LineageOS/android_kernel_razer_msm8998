	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	pdata = dev_get_platdata(&mfd->pdev->dev);
	if (pdata != NULL && pdata->apply_display_setting) {
		ret = pdata->apply_display_setting(pdata, persist_mode);
	}

	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata, panel_data);
	if (ctrl_pdata != NULL && ctrl_pdata->configure_ambient_mode) {
		ret = ctrl_pdata->configure_ambient_mode(ctrl_pdata, persist_mode ? true : false);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	// 0 1 0 1 SRE_Medium
	// 0 1 1 0 SRE_High
	switch (ctrl->ie_sre_mode) {
	case 0:
		ret = scnprintf(buf, PAGE_SIZE, "off\n");
		break;
	case 0x48:
		ret = scnprintf(buf, PAGE_SIZE, "sre_low\n");
		break;
	case 0x58:
		ret = scnprintf(buf, PAGE_SIZE, "sre_medium\n");
		break;
	case 0x68:
		ret = scnprintf(buf, PAGE_SIZE, "sre_high\n");
		break;
	case 0x80:
		ret = scnprintf(buf, PAGE_SIZE, "ie_low\n");
		break;
	case 0x90:
		ret = scnprintf(buf, PAGE_SIZE, "ie_medium\n");
		break;
	case 0xb0:
		ret = scnprintf(buf, PAGE_SIZE, "ie_high\n");
		break;
	default:
		ret = -EINVAL;
		break;
	...
	mutex_unlock(&mfd->ie_sre_lock);
	return ret;
}

static ssize_t mdss_fb_set_ie_sre_mode(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *) fbi->par;
	...
	case 11: // IE High
		...
		break;
	default:
		pr_err("%s: invalid IE/SRE mode %d\n", __func__, mode);
		return -EINVAL;
	}

	/* make sure that we are idle while changing modes */
	ret = mdss_fb_pan_idle(mfd);
	if (ret) {
		pr_err("mdss_fb_pan_idle for fb%d failed. ret=%d\n",
			mfd->index, ret);
	...
	if (mdss_panel_is_power_off(mfd->panel_power_state)) {
		...
		if (mode > 0 && !(mode & 0x80)) {
			// SRE feature
			mode |= 0x08; // SRE_HBM_EN bit
		}

		ctrl->ie_sre_mode = mode;
		goto end;
	}

	if (ctrl->apply_ie_sre_mode)
		ret = ctrl->apply_ie_sre_mode(pdata, mode);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	panel_on = mdss_fb_is_power_on(mfd);
	...
	if (ctrl->night_mode_ctl) {
		...
		ctrl->night_mode_ctl(pdata, mode, panel_on);
...
static DEVICE_ATTR(ie_sre_mode, S_IRUGO | S_IWUSR | S_IWGRP,
		mdss_fb_get_ie_sre_mode, mdss_fb_set_ie_sre_mode);
			     ...
			     int op_enable)
{
	...
	case FB_BLANK_UNBLANK:
		pr_debug("%s: unblank called. cur pwr state=%d\n", __func__, cur_power_state);
		ret = mdss_fb_blank_unblank(mfd);
		break;
	case BLANK_FLAG_ULP:
		req_power_state = MDSS_PANEL_POWER_LP2;
		pr_debug("%s: ultra low power mode requested\n", __func__);
		if (mdss_fb_is_power_off(mfd)) {
			return 0;
		}
	...
	case BLANK_FLAG_LP:
		...
		pr_debug("%s: power mode requested\n", __func__);
